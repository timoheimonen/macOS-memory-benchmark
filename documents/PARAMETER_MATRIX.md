...
| — | `--llm-memory-backend` | `cpu\|metal` | LLM execution backend; default `cpu`; the experimental Metal preview activates decode with contiguous or paged KV |
...
| `--kv-layout <contiguous\|paged>` | ✅ | Default `contiguous`; both layouts are executable for decode and prefill on CPU and for decode on Metal |
...
padding validation. The experimental Metal/decode preview is active for both layouts with Tier 2 argument-buffer
indirection, GPU command-buffer timestamps, dual-mod32 checksum validation, and excluded validation of every byte in
the final K/V append records. Contiguous uses exact-tail W/K/V segments. Paged uses whole-block K/V segmentation,
segmented private table storage, exact `L*B*(2*N+1)` lookup evidence, and terminal-block padding canaries. The command
does not fall back to another backend, phase, or layout.
