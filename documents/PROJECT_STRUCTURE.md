...
| `documents/LLM_MEMORY_PROFILE_WHITEPAPER.md` | Whitepaper: CPU decode/prefill contiguous/paged methodology plus experimental Metal decode for both layouts and prefill for contiguous KV, with exact payload formulas, resources, validation, schema 1, and interpretation limits |
...
Standalone generic schema-1 vocabulary with all four CPU decode/prefill and contiguous/paged implementations plus
experimental Metal decode for both layouts and prefill for contiguous KV. Pure logical phase planning, deterministic
...
hot kernels. Only Metal prefill with paged KV remains inactive, and no unsupported Metal request falls back to CPU.
...
| `llm_metal_backend.h` | Objective-C-free planning, capability, contiguous/paged resource and memory admission, one-owner grid, decode/prefill parameter ABIs, timestamp/checksum mapping, diagnostics, and active-profile test seams |
| `llm_metal_backend.mm` | Objective-C++ Metal boundary for capability probing, selected-profile runtime compilation, Tier 2 argument encoding, private/shared allocation, table upload/validation, block-aligned or exact-tail segmented dispatch, GPU timestamps, layout-aware dual-mod32 checksum, generic K/V-write and padding validation, diagnostics, and idempotent cleanup |
| `llm_metal_kernels_source.h` | Canonical embedded MSL 2.3 source with versioned decode-contiguous, decode-paged, and 136-byte prefill-contiguous parameter ABIs; common foundation entrypoints; scenario-specialized decode and prefill workload entrypoints; named-lane volatile paged-table publication; and phase-specific K/V-write/padding validation; exact selected source bytes are hashed at runtime |
...
| `test_llm_memory_json.cpp` | `LlmMemoryJsonTest` | Schema-1 CPU and active Metal identity, exact nullable geometry/workers, paged layout/permutation, prefill geometry, ownership/segmentation, capability/resource/task K/V-write/lookup/padding evidence, output-peak, status, interpretation, environment, and checkpoint evidence |
| `test_llm_memory_output.cpp` | `LlmMemoryOutputTest` | Exact decode/prefill, contiguous/paged, and Metal device/resource/table/task K/V-write/lookup/padding formatting, interpretation text, and deduplicated warnings |
...
| `test_llm_metal_backend.cpp` | `LlmMetalBackendTest`, `LlmMetalBackendIntegrationTest`, `LlmMetalBackendFailureInjectionIntegrationTest` | Pure capability, contiguous/paged segmentation, argument-buffer, one-owner grid, decode/prefill ABI, source-hash and prefill loop-order audit, phase-aware checksum/reason mapping, and admission coverage plus real-device selected-profile compilation, Tier-2 slots, private table/K/V initialization, exact-tail decode and full-prompt/tiled-prefix prefill scenarios, wrong-table/K/V-write/padding detection, interruption/failure cleanup, and idempotent release |
