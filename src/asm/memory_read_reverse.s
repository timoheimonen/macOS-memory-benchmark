...
// -----------------------------------------------------------------------------
// memory_read_reverse_loop_asm
// -----------------------------------------------------------------------------
// C++ Prototype:
//   extern "C" uint64_t memory_read_reverse_loop_asm(const void* src, size_t byteCount);
// Purpose:
//   Read 'byteCount' bytes from 'src' in reverse order (backwards) to exercise
//   memory read bandwidth with reverse sequential access pattern.
...
//   * 512B main loop mirrors forward read routine structure but processes backwards.
...
.align 4
_memory_read_reverse_loop_asm:
    // Start from end of buffer and work backwards
    add x3, x0, x1          // end_ptr = src + byteCount
...
read_reverse_loop_start_512:        // Main 512B block loop (reverse direction)
    ...
    ldp q4,  q5,  [x7, #0]        // Load pair 0 (offset 0)
    ldp q6,  q7,  [x7, #32]       // Load pair 1 (offset 32)
    ldp q16, q17, [x7, #64]       // Load pair 2 (offset 64)
    ldp q18, q19, [x7, #96]       // Load pair 3 (offset 96)
    ldp q20, q21, [x7, #128]      // Load pair 4 (offset 128)
    ldp q22, q23, [x7, #160]      // Load pair 5 (offset 160)
    ldp q24, q25, [x7, #192]      // Load pair 6 (offset 192)
    ldp q26, q27, [x7, #224]      // Load pair 7 (offset 224)
    ...
    ldp q4,  q5,  [x7, #256]      // Load pair 8 (offset 256, reuse q4-q5)
    ldp q6,  q7,  [x7, #288]      // Load pair 9 (offset 288, reuse q6-q7)
    ldp q16, q17, [x7, #320]      // Load pair 10 (offset 320, reuse q16-q17)
    ldp q18, q19, [x7, #352]      // Load pair 11 (offset 352, reuse q18-q19)
    ldp q20, q21, [x7, #384]      // Load pair 12 (offset 384, reuse q20-q21)
    ldp q22, q23, [x7, #416]      // Load pair 13 (offset 416, reuse q22-q23)
    ldp q24, q25, [x7, #448]      // Load pair 14 (offset 448, reuse q24-q25)
    ldp q26, q27, [x7, #480]      // Load pair 15 (offset 480, reuse q26-q27)
    ...
    b read_reverse_loop_start_512   // Loop again

read_reverse_cleanup:          // Tail handling when <512B remain
    ...
    ldp q16, q17, [x7, #64]   // Load third pair (32B)
    ldp q18, q19, [x7, #96]   // Load fourth pair (32B)
    ldp q20, q21, [x7, #128]  // Load fifth pair (32B)
    ldp q22, q23, [x7, #160]  // Load sixth pair (32B)
    ldp q24, q25, [x7, #192]  // Load seventh pair (32B)
    ldp q26, q27, [x7, #224]  // Load eighth pair (32B)
