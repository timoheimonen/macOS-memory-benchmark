...
std::string usage_options(const std::string& prog_name) {
  ...
  oss << "Usage: " << prog_name << " [options]\n"
      ...
      << "  -M, --llm-memory      Run the standalone CPU synthetic LLM decode/prefill memory profile.\n"
      ...
      << "                        --attention-query-tile-tokens. Decode supports contiguous/paged KV;\n"
      << "                        prefill currently supports contiguous KV only. Use\n"
      ...
      << "                        "
      << Constants::LLM_CPU_DECODE_PAGED_METHODOLOGY_VERSION << ", and\n"
      << "                        "
