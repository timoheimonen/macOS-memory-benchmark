...
std::string usage_options(const std::string& prog_name) {
  ...
  oss << "Usage: " << prog_name << " [options]\n"
      ...
      << "                        Uses two private/tracked buffers, GPU command-buffer timing, and\n"
      << "                        GPU schema " << Constants::GPU_JSON_SCHEMA_VERSION << " methodology "
      << Constants::GPU_METHODOLOGY_VERSION << ".\n"
      << "                        Defaults to " << Constants::GPU_DEFAULT_BUFFER_SIZE_MB
      << " MB per buffer and " << Constants::GPU_DEFAULT_LOOP_COUNT
      << " balanced loops; minimum buffer size is "
      << Constants::GPU_MIN_BUFFER_SIZE_MB << " MB.\n"
      << "  -M, --llm-memory      Configure the standalone CPU synthetic LLM decode-memory profile.\n"
      << "                        Required: --weight-size-mb, --layers, --query-heads, --kv-heads,\n"
      << "                        --head-dim, and --context-tokens. Use --llm-memory --help for\n"
      << "                        its exact whitelist, defaults, and memory-only interpretation.\n"
      << "                        This build validates the command boundary; execution is unavailable.\n"
      << "  -i, --iterations <count>\n"
      << "                        Exact measured R/W/Copy pass count when explicitly supplied.\n"
      << "                        When omitted, --benchmark, --patterns, and --gpu-bandwidth\n"
      << "                        calibrate operation work toward 150 ms. The future LLM executor\n"
      << "                        uses the same target for scenario-specific calibration.\n"
      ...
      << "                        Standalone GPU defaults to 3 balanced loops; LLM-memory stores\n"
      << "                        3 planned loops for its future executor.\n"
      ...
      << "                        --gpu-bandwidth, and --llm-memory, or planner, round-order, and pointer-chain\n"
      << "                        seed for --analyze-tlb.\n"
      << "                        Generated once per command when omitted.\n"
      << "  -C, --analyze-core2core\n"
      << "                        Run calibrated, balanced two-thread acquire/release token-handoff analysis.\n"
      << "                        Round trips include protocol, coherence, and scheduler effects.\n"
      ...
      << "                        In general CPU modes, an explicit value applies to every bandwidth\n"
      << "                        target; latency remains single-threaded, and requests above available\n"
      << "                        cores are capped. LLM-memory uses detected CPU workers when omitted\n"
      << "                        and keeps an explicit request uncapped for later effective reduction.\n"
      ...
      << "                        for every result-producing direct mode and "
         "CPU sweep and routes human output to stderr;\n"
      ...
      << "                        The current LLM boundary accepts this syntax but creates no result.\n"
      ...
      << "                        GPU and LLM-memory standalone modes do not support sweeps.\n"
      << "  -X, --sweep-max-runs <n>\n"
