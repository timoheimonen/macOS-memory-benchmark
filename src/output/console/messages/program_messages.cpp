...
std::string usage_options(const std::string& prog_name) {
  ...
  oss << "Usage: " << prog_name << " [options]\n"
      ...
      << "Options:\n"
      << "  -B, --benchmark       Run standard memory benchmark (bandwidth and latency).\n"
      << "  -i, --iterations <count>\n"
      << "                        Number of iterations for standard R/W/Copy tests (default: " << Constants::DEFAULT_ITERATIONS << ").\n"
      << "                        With --patterns, an explicit value overrides automatic 150 ms calibration.\n"
      ...
      << "      --seed <uint64>\n"
      << "                        Reproducible random workload seed for --patterns, or planner,\n"
      << "                        round-order, and pointer-chain seed for --analyze-tlb.\n"
