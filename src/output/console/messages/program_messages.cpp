...
std::string usage_options(const std::string& prog_name) {
  ...
  oss << "Usage: " << prog_name << " [options]\n"
      ...
      << "                        When omitted, --benchmark, --patterns, and "
         "--gpu-bandwidth calibrate toward 150 ms.\n"
      ...
      << "                        and --gpu-bandwidth, or planner, round-order, and pointer-chain\n"
      ...
      << "                        (allows optional -o/--output <target>, -r/--count <count>, -n/--latency-samples <count>,\n"
      ...
      << "  -o, --output <target> JSON output target. For direct CPU modes, exact - writes one final\n"
      << "                        JSON document to stdout and routes human output to stderr; ./- and\n"
      << "                        all other values are files. Standard files checkpoint atomically;\n"
      << "                        other direct CPU files write atomically once. GPU and sweeps require files.\n"
