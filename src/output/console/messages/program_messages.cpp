...
std::string usage_options(const std::string& prog_name) {
  ...
  oss << "Usage: " << prog_name << " [options]\n"
      ...
      << "                        Required common inputs: --weight-size-mb, --layers, --query-heads,\n"
      ...
      << "                        or paged KV on CPU. The experimental Metal preview accepts only\n"
      << "                        decode with contiguous KV and never falls back to CPU. Its M4\n"
      << "                        validation gate passed; Apple7/M1 baseline validation is pending. Use\n"
      ...
      << "                        "
      << Constants::LLM_CPU_DECODE_PAGED_METHODOLOGY_VERSION << ",\n"
      << "                        "
      << Constants::LLM_CPU_PREFILL_CONTIGUOUS_METHODOLOGY_VERSION << ",\n"
      << "                        "
      << Constants::LLM_CPU_PREFILL_PAGED_METHODOLOGY_VERSION << ", and\n"
      ...
      << "                        --llm-memory calibrate operation/scenario work toward 150 ms.\n"
      ...
      << "                        Standalone GPU and LLM-memory default to 3 balanced loops.\n"
      ...
      << "                        cores are capped. CPU LLM-memory uses detected workers when omitted\n"
