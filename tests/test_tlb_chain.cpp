...
class PageBuffer {
 public:
  explicit PageBuffer(size_t size_bytes) : size_bytes_(size_bytes) {
    pointer_ = mmap(nullptr, size_bytes_, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
...
std::vector<size_t> traversal_offsets(void* buffer, void* head, size_t node_count) {
...
uintptr_t address_at(void* buffer, size_t offset) { return reinterpret_cast<uintptr_t>(buffer) + offset; }
...
TlbChainDiagnostics expected_chain(size_t page_size, size_t node_count, size_t requested_pages,
                                   TlbChainLayout layout = TlbChainLayout::Spread) {
  TlbChainDiagnostics expected;
  expected.layout = layout;
  expected.traversal_policy = TlbChainTraversalPolicy::RandomPagesRandomOffsets;
...
TEST(TlbChainTest, LayoutSeedDerivationMatchesGoldenValues) {
  EXPECT_EQ(derive_tlb_chain_layout_seed(42, TlbChainLayout::Spread), 0xea98f6ee088b8b6cULL);
  EXPECT_EQ(derive_tlb_chain_layout_seed(42, TlbChainLayout::Packed), 0x79f604e46045df73ULL);
...
TEST(TlbChainTest, SpreadVisitsExactlyEveryRequestedPage) {
  ...
  const TlbChainBuildResult result =
      build_tlb_chain(buffer.get(), buffer.size(), kRequestedPages, page_size, 256, TlbChainLayout::Spread,
                      TlbChainTraversalPolicy::RandomPagesRandomOffsets, 1234);
  ...
  EXPECT_EQ(validate_tlb_chain(buffer.get(), buffer.size(), result.chain_head, result.diagnostics),
...
TEST(TlbChainTest, PackedControlPreservesNodeAndCacheLineCounts) {
  ...
  const TlbChainBuildResult spread =
      build_tlb_chain(buffer.get(), buffer.size(), kRequestedPages, page_size, 256, TlbChainLayout::Spread,
                      TlbChainTraversalPolicy::RandomPagesRandomOffsets, 111);
  ASSERT_EQ(spread.status, TlbChainBuildStatus::Success);

  const TlbChainBuildResult packed =
      build_tlb_chain(buffer.get(), buffer.size(), kRequestedPages, page_size, 256, TlbChainLayout::Packed,
                      TlbChainTraversalPolicy::RandomPagesRandomOffsets, 222);
  ASSERT_EQ(packed.status, TlbChainBuildStatus::Success);

  EXPECT_EQ(packed.diagnostics.node_count, spread.diagnostics.node_count);
  EXPECT_EQ(packed.diagnostics.unique_cache_lines, spread.diagnostics.unique_cache_lines);
  EXPECT_LT(packed.diagnostics.actual_pages, spread.diagnostics.actual_pages);
  ...
  constexpr size_t kPageStrideRequestedPages = 32;
  PageBuffer page_stride_buffer(kPageStrideRequestedPages * page_size);
  ASSERT_NE(page_stride_buffer.get(), nullptr);
  const TlbChainBuildResult page_stride_packed =
      build_tlb_chain(page_stride_buffer.get(), page_stride_buffer.size(), kPageStrideRequestedPages, page_size,
                      page_size, TlbChainLayout::Packed, TlbChainTraversalPolicy::RandomPagesRandomOffsets, 321);

  ASSERT_EQ(page_stride_packed.status, TlbChainBuildStatus::Success);
  EXPECT_EQ(page_stride_packed.diagnostics.node_count, kPageStrideRequestedPages);
  EXPECT_EQ(page_stride_packed.diagnostics.unique_cache_lines, kPageStrideRequestedPages);
  EXPECT_EQ(page_stride_packed.diagnostics.actual_pages, 1U);
  EXPECT_EQ(page_stride_packed.diagnostics.effective_node_spacing_bytes, 64U);
}
...
TEST(TlbChainTest, SameSeedReproducesTraversalAndDifferentSeedChangesIt) {
  ...
  auto build_offsets = [&](uint64_t seed) {
    const TlbChainBuildResult result =
        build_tlb_chain(buffer.get(), buffer.size(), kRequestedPages, page_size, 128, TlbChainLayout::Spread,
                        TlbChainTraversalPolicy::RandomPagesRandomOffsets, seed);
...
TEST(TlbChainTest, ValidationRejectsPointerOutsideBuffer) {
  ...
  const TlbChainBuildResult result =
      build_tlb_chain(buffer.get(), buffer.size(), kRequestedPages, page_size, 64, TlbChainLayout::Spread,
                      TlbChainTraversalPolicy::RandomPagesRandomOffsets, 55);
  ASSERT_EQ(result.status, TlbChainBuildStatus::Success);

  const uintptr_t outside = reinterpret_cast<uintptr_t>(buffer.get()) + buffer.size();
...
TEST(TlbChainTest, ValidationReportsInvalidArgumentBeforeTraversal) {
  ...
  EXPECT_EQ(validate_tlb_chain(nullptr, buffer.size(), buffer.get(), expected),
            TlbChainValidationStatus::InvalidArgument);
  EXPECT_EQ(validate_tlb_chain(buffer.get(), buffer.size(), nullptr, expected),
            TlbChainValidationStatus::InvalidArgument);
  TlbChainDiagnostics zero_nodes = expected;
  zero_nodes.node_count = 0;
  EXPECT_EQ(validate_tlb_chain(buffer.get(), buffer.size(), buffer.get(), zero_nodes),
            TlbChainValidationStatus::InvalidArgument);
}
...
TEST(TlbChainTest, RejectsInvalidStrideAndInsufficientSpreadBuffer) {
  ...
  EXPECT_EQ(build_tlb_chain(buffer.get(), buffer.size(), 4, page_size, 10, TlbChainLayout::Spread,
                            TlbChainTraversalPolicy::RandomPagesRandomOffsets, 1)
                .status,
            TlbChainBuildStatus::InvalidArgument);
  EXPECT_EQ(build_tlb_chain(buffer.get(), buffer.size(), 4, page_size, 64, static_cast<TlbChainLayout>(99),
                            TlbChainTraversalPolicy::RandomPagesRandomOffsets, 1)
                .status,
            TlbChainBuildStatus::InvalidArgument);
  EXPECT_EQ(build_tlb_chain(buffer.get(), buffer.size(), 5, page_size, 64, TlbChainLayout::Spread,
                            TlbChainTraversalPolicy::RandomPagesRandomOffsets, 1)
                .status,
...
TEST(TlbChainTest, AcceptsAlignedStrideThatDoesNotDividePage) {
  ...
  const TlbChainBuildResult result =
      build_tlb_chain(buffer.get(), buffer.size(), 8, page_size, 136, TlbChainLayout::Spread,
                      TlbChainTraversalPolicy::RandomPagesRandomOffsets, 123);
  ASSERT_EQ(result.status, TlbChainBuildStatus::Success);
...
TEST(TlbChainTest, EveryTraversalPolicyBuildsAValidSpreadChain) {
  ...
  for (TlbChainTraversalPolicy policy : {
    ...
    const TlbChainBuildResult result = build_tlb_chain(buffer.get(), buffer.size(), kRequestedPages, page_size, 256,
                                                       TlbChainLayout::Spread, policy, 1234);
...
TEST(TlbChainTest, ScratchReusePreservesSemanticsWithoutGrowingCapacity) {
  ...
  const TlbChainBuildResult first =
      build_tlb_chain(reused_buffer.get(), reused_buffer.size(), 64, page_size, 256, TlbChainLayout::Spread,
                      TlbChainTraversalPolicy::RandomPagesRandomOffsets, 1234, scratch);
  ...
  const TlbChainBuildResult reused =
      build_tlb_chain(reused_buffer.get(), reused_buffer.size(), 16, page_size, 256, TlbChainLayout::Spread,
                      TlbChainTraversalPolicy::IncreasingPagesRandomOffsets, 5678, scratch);
  ASSERT_EQ(reused.status, TlbChainBuildStatus::Success);
  const std::vector<size_t> reused_offsets =
      traversal_offsets(reused_buffer.get(), reused.chain_head, reused.diagnostics.node_count);

  TlbChainScratch fresh_scratch;
  const TlbChainBuildResult fresh =
      build_tlb_chain(fresh_buffer.get(), fresh_buffer.size(), 16, page_size, 256, TlbChainLayout::Spread,
                      TlbChainTraversalPolicy::IncreasingPagesRandomOffsets, 5678, fresh_scratch);
  ASSERT_EQ(fresh.status, TlbChainBuildStatus::Success);
  const std::vector<size_t> fresh_offsets =
      traversal_offsets(fresh_buffer.get(), fresh.chain_head, fresh.diagnostics.node_count);
  ...
  EXPECT_EQ(reused.diagnostics.unique_cache_lines, fresh.diagnostics.unique_cache_lines);
  EXPECT_EQ(reused.diagnostics.max_nodes_per_page, fresh.diagnostics.max_nodes_per_page);
  EXPECT_EQ(reused.diagnostics.byte_span, fresh.diagnostics.byte_span);
  EXPECT_EQ(reused.diagnostics.effective_node_spacing_bytes, fresh.diagnostics.effective_node_spacing_bytes);
