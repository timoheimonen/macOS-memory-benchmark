...
### Added
  ...
  - **Standalone TLB JSON schema version 2**: TLB configuration metadata identifies `schema_version: 2`, `methodology_version: "locality-sweep-v2-balanced-rounds"`, base seed/source, schedule policy, requested and effective chain modes, and macOS version. Point metadata and execution-ordered measurement records expose pass, round, order, derived seed, and raw latency. The analyzer plot example accepts both the current schema and historical result files.
