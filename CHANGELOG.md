...
### Changed
  ...
  - **Repeatability-oriented pattern thread default**: `--patterns` defaults to the detected performance-core count to avoid an implicit P/E-core mixture. Explicit `--threads N` still selects any detected-core count, while JSON documents that QoS is a best-effort scheduler hint and does not pin workers to cores.
