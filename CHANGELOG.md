...
### Added
  ...
  - **Standalone TLB JSON schema version 3**: TLB configuration metadata identifies `schema_version: 3`, `methodology_version: "page-native-paired-v3"`, base seed/source, schedule policy, requested and effective chain modes, and macOS version. Execution-ordered records include both raw pair latencies, pair order, layout seeds, chain diagnostics, and same-round translation delta. Point records include spread/packed raw arrays and P50 values, paired-delta arrays/P50, actual page/node/cache-line counts, and pair/raw completion counts. The analyzer plot example accepts current and historical result files.
