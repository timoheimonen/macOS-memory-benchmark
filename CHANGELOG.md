...
### Changed
  - **Default latency profile is now DRAM-first (`512B` stride, global-random chain)**: Changed defaults from `-latency-stride-bytes 64` / `-latency-tlb-locality-kb 16` to `-latency-stride-bytes 512` / `-latency-tlb-locality-kb 0`. This reduces adjacent-line and same-page reuse bias in headline latency and better reflects off-chip memory latency on modern Apple Silicon. The same defaults are inherited by standalone `-analyze-tlb` unless explicitly overridden.
  ...
  - **Documentation synchronized with the new latency baseline**: Updated CLI help and docs to match default `-latency-stride-bytes 512` and default `-latency-tlb-locality-kb 0` behavior (`README.md`, `MANUAL.md`, `LATENCY_WHITEPAPER.md`, `TECHNICAL_SPECIFICATION.md`).
