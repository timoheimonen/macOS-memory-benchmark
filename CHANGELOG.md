...
### Added
  - **Shared command-output transport boundary**: Added raw-value JSON target classification, current-directory and preserve-raw file policies, a non-copyable command-scoped output session, lazy checkpoint dispatch, and checked final stdout serialization. The session can route human `std::cout` output to `std::cerr` while retaining the original stdout buffer for a two-space-indented JSON document with one trailing newline; stream, flush, serialization, and checkpoint-builder failures are contained as return codes. Mode enablement is recorded separately below.
  - **Machine-readable stdout for direct standard and pattern commands**: Exact `--output -` now emits one final schema-3 standard or schema-3 pattern JSON document on stdout while routing the post-parse human transcript to stderr. Stdout checkpoints remain lazy no-ops, representable interrupted or failed state is retained, file targets keep their atomic checkpoint and final-write behavior, and `--output ./-` remains an ordinary file target. The new `API.md` defines the initial process contract, completeness checks, stream rules, and the modes enabled in this first vertical slice.
  - **Machine-readable stdout for direct TLB and core-to-core commands**: Exact `--output -` now emits one terminal schema-4 TLB or schema-2 core-to-core payload while routing runtime human output to stderr. Execution status remains independent of the retained payload, so initialized partial, interrupted, TLB error, and core failure evidence is written before the command returns; setup failures before result initialization leave stdout empty. Real file targets retain their single atomic terminal write.
  - **Machine-readable stdout for CPU parameter sweeps**: Standard, pattern, TLB, and core-to-core sweeps now accept exact `--output -` and emit one final schema-1 envelope while routing their human transcript to stderr. Logical checkpoint transitions remain active but lazy stdout checkpoints neither build nor serialize intermediate payloads. Real-file sweeps retain one atomic checkpoint after every attempted run without an added terminal rewrite or retry, and `--output ./-` remains a file.
  - **Machine-readable stdout for direct GPU bandwidth**: Exact `--output -` now emits one terminal top-level GPU schema-1 document and routes the human transcript to stderr. Initialized unsupported and failed results remain auditable with their non-zero process status, while parser/config and backend-factory failures before result initialization leave stdout empty. GPU stdout executes logical checkpoint transitions and post-checkpoint stop reads without intermediate construction or serialization; real-file targets retain their terminal-measurement/failure checkpoints and post-release replacement, `configuration.output_file` preserves the raw `"-"`, and `--output ./-` remains a file.

### Changed
  - **Public schema contracts and release lineage are evidence-backed**: The API, manual, README, technical references, CLI help, public C++ contracts, and bundled-consumer guidance now distinguish current standard schema 3 from the supported released schema-2 input contract and explicitly reject unknown standard versions. The dated `0.61.1` and `0.61.2` sections remain separate because those versions were published through public `main` merge commits `0cbeb3a` and `fc5338c`; GitHub tags/releases and the Homebrew channel still end at `v.0.61.0`, and no retrospective external release objects were created as part of this change.
  - **Machine-output contracts close the post-review gaps**: CLI help and process/API documentation now distinguish exact stdout sentinel `-`, disabled empty direct targets, invalid empty sweep targets, and every other non-empty file target including `./-` and flag-shaped names. Consumer guidance requires concurrent stdout/stderr draining (or a `communicate` equivalent), keeps the schema-1 sweep acceptance predicate at `status: complete` plus `conclusions_valid: true`, and treats count equality as a separate producer invariant. Standard/pattern builders and legacy file-only C++ adapters now document schema authority, ownership, lifetime, exceptions, concurrency, and target preconditions without duplicate runner parameter documentation.
  - **Flag-shaped output values remain opaque throughout general parser pre-scans**: The primary-mode, TLB-discovery, and cache-size pre-scans consume exactly one existing raw `-o`/`--output` value before option matching, so file targets such as `-G`, `-T`, `--analyze-tlb`, `-k`, and `--cache-size` are preserved byte-for-byte while genuine distinct mode and cache options remain recognized in either order. Missing and duplicate output diagnostics remain owned by the normal parsing passes, and dedicated core-to-core and GPU parser regressions continue to lock their centralized message contracts.
  - **Standard schema 3 carries mandatory transport and completion metadata**: Every current standard payload advances to schema 3, requires string `configuration.output_file`, and emits boolean top-level `conclusions_valid` exactly equal to `results_complete`. Direct results preserve the raw output token, nested sweep results keep an empty target because the envelope owns persistence, and version-aware sweep classification applies the released schema-2 optional-field rule separately from the stricter schema-3 predicate. File, stdout, checkpoint, retained terminal, and nested-sweep regressions lock the new contract without changing the standard methodology identity or measured work.
  - **Standard interruption gives checkpoint failure output precedence**: A stop observed before the next standard loop checks its logical checkpoint result. Failure records the centralized checkpoint-failed state, retains completed evidence and counters, returns failure, and cannot enter the command boundary's file-final write; graceful stdout checkpoints remain lazy no-ops and one interrupted schema-3 terminal document is emitted.
  - **Bundled standard-result readers preserve released schema-2 compatibility**: Shared dependency-free Python and jq validators distinguish released schema 2, current schema 3, and the explicitly supported unversioned historical layout. Schema 2 accepts complete results that omit the later `conclusions_valid` and `configuration.output_file` fields while rejecting false or wrongly typed optional values; schema 3 requires both fields and the full three-field completion predicate. The four bundled latency consumers now use this version-aware dispatch, reject unknown explicit schemas, and are covered by a minimized released-shape fixture plus public-entry regression tests. The manual's `jq -e` examples continue to enforce every mode's schema and completeness predicate, and the API and mode references record observable stdout-transfer failures, zero-attempt sweep checkpoints, and the complete schema-1 sweep example.
  - **Internal output-lifecycle documentation matches orchestration order**: Sweep wrapper contracts distinguish contained nested-run exceptions from propagating setup failures, and the output-session contract permits human `std::cout` messages after the single final stdout write because routing remains active until session destruction.
  - **Sweep executor exceptions retain terminal evidence**: Typed and unknown nested-run exceptions are converted to a stable failed attempt with a null nested result, preserving prior completed runs and the initialized sweep envelope. The normal logical checkpoint still runs once, checkpoint failure keeps precedence without a retry, and stdout commands can emit the retained terminal envelope before returning failure.
  - **Machine-output boundary contracts are explicit**: Stdout stream and checkpoint-builder failure details now come from the centralized message catalog, and public session, target, core-to-core, and GPU JSON interfaces document ownership, lifetime, exception, and transport behavior. Dedicated GPU help now states that human output moves to stderr for exact `-` and that exact `./-` remains a file target.
  - **TLB JSON construction is independent of persistence**: Extracted the schema-4 TLB payload builder from its file writer so callers can retain complete, partial, interrupted, or error evidence in memory before choosing a transport. Existing file output remains delegated to the same atomic writer and retains its schema and path behavior.
  - **Sweep envelopes are versioned and retain initialized failure evidence**: General and core-to-core sweep checkpoints now identify their shared envelope with `configuration.sweep_schema_version: 1`. Standard points build their initialized terminal payload before propagating runner failure, TLB points reuse the in-memory schema-4 collector instead of writing and reparsing a temporary file, and the coordinator classifies every available nested payload before forcing a non-zero run to failed. TLB `status: "error"` maps to a failed attempt with the original payload retained and without inventing a nested `status_reason`; this internal correctness slice preserved the pre-existing file checkpoint cadence before stdout transport was enabled at the command boundary.
  - **GPU checkpoint state is separated from persistence transport**: The backend-independent runner can now offer every logical checkpoint to a command-owned output session while test hooks retain precedence. Lazy stdout sessions preserve the same state transitions and post-checkpoint stop reads without writing intermediate documents; legacy and session-backed file paths retain their existing terminal-measurement checkpoints, post-release replacements, write-failure precedence, and hook-only cadence. The standalone command boundary consumes the retained terminal state for its single stdout document.
  - **Machine-readable CLI API release contract established**: Reserved exact `--output -` as the final-only stdout JSON transport for the upcoming process API, fixed its serialization policy at two-space indentation with one trailing newline, and selected additive `configuration.sweep_schema_version: 1` for both sweep envelopes. Mode-specific schema locations, file-output checkpoint behavior, benchmark methodology, and software version remain stable; the deliberate standard schema-3 advance is described above, while the other mode schema versions are unchanged.
...
### Added
  ...
  - **Duration-calibrated core-to-core work**: Every scheduler-hint scenario now runs an excluded 100,000-round-trip pilot after a 1,000,000-round-trip calibration warmup. The pilot independently calibrates the final warmup toward 25 ms, the continuous headline toward 250 ms, and each separate sample window toward 1 ms while preserving the 20,000/1,000,000/2,000 round-trip minimums.
  - **Auditable standard benchmark measurements**: `--benchmark` results now carry explicit measured, skipped, interrupted, invalid, or failed state plus exact work metadata, so partial operations are represented as unavailable instead of numeric zeroes. Standard runs also track planned/completed loops, phases, and measurements and checkpoint JSON after completed loops when `--output` is active.
  - **Deterministic standard benchmark planning**: Added pure bandwidth/latency work planning, overflow-safe payload accounting, complete-chain-cycle rounding, bounded calibration arithmetic, stable domain-separated seed derivation, and cyclic scheduling helpers with deterministic unit coverage.
  - **Reproducible standard latency workloads**: `--seed <uint64>` now applies to `--benchmark`; an omitted non-zero seed is generated once per command and derived target-specific seeds rebuild equivalent main, L1, L2, custom, sampling, and locality-comparison chains across `--count` loops.
  - **Auditable standard benchmark JSON schema v2**: Standard output now uses methodology `benchmark-v2-calibrated-seeded-balanced` and records command completion, per-loop phase/operation order, nullable measurement state, exact pass/access/payload work, requested/effective workers, decimal-string seeds, pilot/final durations, calibration quality, pooled-sample loop boundaries, and median/CV/MAD aggregate quality metadata.
  - **Standard ARM64 kernel contract matrix**: Apple Silicon integration coverage now validates main/cache read, write, copy, and latency kernels for AAPCS64 callee-saved registers, protected access boundaries, 1/31/32/511/512/513-byte tails, source preservation, complete destination coverage, and full two-lane-plus-tail read checksums.
  - **Deterministic standard failure seams**: Kernel-free tests now inject timer creation failure, checkpoint failure, stops between loops, preparation failure at every phase boundary, invalid/non-finite elapsed values, and worker startup failure. The production phase scheduler and elapsed validator consume the same tested helpers.
...
### Changed
  - **Balanced and robust repeated core-to-core summaries**: The three scheduler-hint scenarios rotate in cyclic Latin-square order across `--count` loops. Each scenario's headline is the median P50 of completed continuous loop measurements; repeatability statistics now include CV and MAD, warn above 7.5% headline CV without filtering values, and keep pooled sample-window statistics explicitly separate from headline repeatability.
  - **Core-to-core default count increased to three**: Bare `-C` / `--analyze-core2core` now measures three loops per scenario, producing a median headline plus CV/MAD repeatability metadata by default. The general `--count` default remains one outside core-to-core mode; with 1,000 sample windows targeting about 1 ms each, the deeper core-to-core default intentionally takes longer.
  - **Core-to-core duration and scheduler audit metadata**: Continuous headline windows are classified against the 100-300 ms intended window, thread-hint outcomes are retained for every loop, and affinity-scenario comparison is declared interpretable only for a complete command whose requested hints were applied in every measured record.
  - **Resumable core-to-core sweeps**: Core-to-core sweep JSON is atomically checkpointed after every run with `status`, `planned_runs`, `completed_runs`, and `conclusions_valid`; repeated sweep keys such as two `--sweep count=...` specifications are rejected instead of producing an ambiguous Cartesian plan.
  - **Duration-calibrated standard benchmark**: Omitted `--iterations` selects bounded per-target/per-operation duration calibration toward 150 ms, while an explicit value remains the exact pass count. Latency headlines use a separate continuous pass calibrated toward 250 ms and rounded to complete pointer-chain cycles; calibrated work is reused across repeated loops.
  - **Observed standard worker QoS metadata**: Every standard bandwidth measurement now records requested and created workers, per-worker QoS successes/failures, and a normalized all/partial/failed outcome. Main-thread latency records the corresponding main-thread QoS result, preserving the best-effort scheduler policy without implying core pinning.
  - **Paired standard locality comparison**: The automatic main-memory comparison now alternates first-measured 16 KiB-locality and global-random layouts by round, uses deterministic layout seeds, and aggregates same-round latency deltas instead of subtracting independently grouped medians.
  - **Conservative standard locality terminology**: Console and schema v2 now report `locality_16k_latency_ns`, `global_random_latency_ns`, and `locality_latency_delta_ns`; the standard benchmark no longer labels this mixed locality/cache comparison as an isolated page-walk penalty and leaves validated translation conclusions to `--analyze-tlb`.
  - **Robust repeated standard summaries**: Multi-loop standard aggregates use median P50 as the headline and report average, P90/P95/P99, sample standard deviation, minimum/maximum, coefficient of variation, and median absolute deviation without filtering outliers. CV above the hardware-validated 7.5% threshold produces a diagnostic warning without invalidating the aggregate.
  - **Latency populations separated in console summaries**: Repeatability statistics and CV warnings now come only from continuous per-loop latency headlines. Optional sample-window values are printed in a separately labeled pooled distribution, and interrupted summaries distinguish requested loops from loops with realized measurements.
  - **Hardware-validated latency duration window**: Continuous latency remains targeted at 250 ms, with a distinct 100–300 ms acceptance window instead of sharing bandwidth's 100–250 ms window. A required 16-cycle minimum that itself exceeds the window is reported as `minimum-complete-cycles-exceed-window`.
  - **Apple M4 stability validation closure**: The standard benchmark matrix covers full, bandwidth-only, latency-only, automatic, explicit, sweep, cache-disable, custom-cache, non-cacheable, reproducibility, sample-depth, and interruption paths. Excluding two explicitly classified minimum-cycle cases, 162 of 163 automatic measurements (99.39%) landed in their intended duration window; raw JSON and the validation record remain under `plans/`.
  ...
  - **Standard benchmark documentation and examples synchronized**: CLI help, README, manual, capabilities, parameter matrix, technical specification, and bundled latency/CPU-comparison scripts now describe calibrated seeded execution, continuous latency headlines, paired locality terminology, completion states, and standard JSON schema v2 while retaining explicitly isolated legacy-reader fallbacks where needed.

### Fixed
  - **Core-to-core worker startup failures cannot strand a peer**: Thread creation now has a cancellation-aware start gate; if initiator startup fails after responder creation, the responder is released and joined safely. Direct core-to-core runs also prepare and restore the benchmark signal mask, and failed or interrupted commands preserve auditable partial JSON when an output file was requested.
  - **Incremental builds track all project headers**: Makefile production and test C++ object rules now depend on every `src/**/*.h` file, preventing stale objects from retaining an incompatible `BenchmarkConfig` or result-structure layout after header changes.
  - **Standard worker startup failures are explicit**: Parallel standard bandwidth setup now catches thread-creation failures, releases and joins workers already created, suppresses the invalid numeric measurement, and serializes an explicit failed/startup state instead of terminating or reporting partial work as valid bandwidth.
  - **Automatic standard work is labeled correctly at startup**: The configuration summary no longer prints the unused legacy default as `Iterations: 1000` when `--iterations` is omitted. It now reports the active bandwidth and continuous-latency calibration targets, windows, and latency cycle minimum.
  - **Cache read checksum folds both vector lanes**: `memory_read_cache_loop_asm` now XOR-folds both 64-bit lanes of the final NEON accumulator before the byte tail, matching the corrected main-memory read checksum contract.
...
### Added
  ...
  - **Page-native TLB chains and paired controls**: Every standalone TLB task builds a verified spread chain with exactly one node per requested page and a cache-line-dense packed control with the same node and unique-cache-line counts. The pair runs in one scheduler round with derived layout seeds and alternating first-measured layout. Pointer values are written in physical-slot order, and chain validation requires an in-bounds full cycle that returns to its head. Paired summaries expose spread/packed P50 values, the median same-round translation delta, active cache-line footprint, and a short-cycle diagnostic for chains with fewer than 64 nodes. Console rendering displays sub-resolution negative deltas as `0.00 ns`.
  - **Explicit TLB-analysis completion metadata**: Standalone `--analyze-tlb` JSON includes status, point counts, adaptive round bounds, per-pass realized rounds/convergence/completion reasons, planned measurement bounds, completed measurement counts, and `conclusions_valid`. Combined sweep JSON includes `status`, `planned_runs`, `completed_runs`, and `conclusions_valid`, including a `failed` checkpoint state when a sweep run fails.
  ...
  - **Validated standalone TLB JSON schema version 4**: TLB configuration metadata identifies `schema_version: 4`, `methodology_version: "page-native-paired-adaptive-validated-v4"`, runtime profile and calibration policy, adaptive round bounds, work and memory estimates, best-effort QoS and buffer-lock status, the translation-delta boundary signal, deterministic bootstrap method, seed source/derivation rules, schedule policy, requested/effective chain modes, and macOS version. Every base, task, and layout `seed` is an exact uint64 decimal string. Execution-ordered records include per-chain pilot timing, calibrated accesses, requested/actual pages, pointer-node density, physical diagnostics, and paired data. Explicit base+validation, large-locality, and total counters remove pass-scope ambiguity; `validation_required`, `validation_status`, and `validation_complete` distinguish complete, not-required, not-run, partial, and interrupted validation. Boundary objects contain the accepted bracket, discovery/validation evidence and 95% CIs, while accepted and rejected candidates remain available for diagnosis. The sole 512 MiB result object is `large_locality_paired_comparison`, whose P50 is the median of same-round spread-minus-packed differences and is explicitly neither DRAM latency nor an isolated page-table-walk cost. Schema 4 contains only current field names and no alias or compatibility fields.
  - **Resource-bounded TLB execution profiles**: The default `medium`/standard profile calibrates each chain toward a 10 ms measurement while preserving at least 16 whole-chain cycles. `low`/quick and `high`/exhaustive provide explicit shorter and deeper policies. Quick-profile conclusions are visibly labeled as screening estimates that require medium/high confirmation. Buffer selection admits the largest `1024/512/256 MB` candidate whose predicted buffer-plus-scratch peak fits a conservative available-memory budget, reports the work estimate before each pass, reuses chain/bootstrap scratch storage, and records `mlock()` errno while continuing unlocked under the documented best-effort policy. Analyze-TLB Cartesian sweeps default to a separate 16-run guardrail; explicit `--sweep-max-runs` values still override it.

### Changed
  - **Robust translation-delta boundary model**: L1/L2 boundary inference uses a round-by-point matrix of same-round spread-minus-packed deltas. A candidate must exceed a predefined `0.5ns` effect, have a deterministic bootstrap 95% CI above the measured noise floor, persist at both following locality points, and repeat in an independent validation pass. A single transient spike is rejected, uncertainty brackets are the primary entry result, and midpoint values remain explicitly labeled estimates.
  - **Page-consistent standalone TLB locality grid**: `--analyze-tlb` now rejects strides larger than the system page size. Fine-sweep refinement points are page-aligned and exclude already measured base-grid localities. Planner pointer counts now represent requested active pages rather than stride-derived pointer slots.
  - **Concise standalone TLB reporting**: Existing verbose settings and result sections are replaced by compact run/resource/sweep summaries. Per-locality console output now leads with the paired translation delta, while detailed measurement, calibration, scheduling, and memory diagnostics remain in JSON. Legacy `configuration.accesses_per_loop` is replaced by calibrated per-pair `access_count` values.
  - **TLB and sweep documentation synchronized with the shipped contract**: CLI help, README, manual, capabilities overview, parameter matrix, technical specification, TLB analysis whitepaper, JSON examples, and the analyzer plot script document page-native paired chains, stride constraints, seeded balanced rounds, independent validation, bootstrap confidence intervals, completion states, checkpoints, conclusion suppression, and the clean schema version 4 contract. The plotter independently retains read support for historical schema 1-3 files but accepts only current field names when `schema_version` is `4`.
...
### Fixed
  ...
  - **Complete sweep preflight validation**: Duplicate sweep parameter keys are rejected, and every generated Cartesian configuration is validated before the first benchmark run.
  - **Unambiguous incomplete TLB results**: Interrupted or partial standalone analyses retain completed measurements but suppress private-cache, L1/L2, and large-locality conclusions in console and JSON output. Validation that never ran reports `validation_status: "not-run"` with `validation_complete: false`; a complete analysis with no required validation reports `not-required`. Empty interrupted sweeps serialize safely instead of being mistaken for complete analyses.
...
### Changed
  ...
  - **`-analyze-tlb` base sweep density increased (15 -> 29 canonical points)**: Standalone TLB analysis now uses a denser default locality grid from `16KB` to `256MB` (still stride-clamped by `max(16KB, 2*stride)`), then performs the existing refinement pass around detected transitions. This reduces boundary quantization and improves L1/L2 boundary localization at the cost of longer analysis runtime.
  - **`-analyze-tlb` boundary threshold now adapts to measured loop noise**: Boundary detection still enforces hard floors (`>= 2.0ns` and `>= 10%` of baseline), but now also includes a baseline-noise term derived from median IQR of baseline loop latencies. On noisier runs this raises the threshold automatically and reduces false-positive boundary detections.
...
### Changed
  - **Default latency profile updated to `256B` stride with `1MB` locality window**: Changed defaults from `-latency-stride-bytes 64` / `-latency-tlb-locality-kb 16` to `-latency-stride-bytes 256` / `-latency-tlb-locality-kb 1024`. This reduces adjacent-line prefetch bias from the old `64B` stride while avoiding the heavier page-turnover bias of `512B`, keeping a locality-preserving baseline for more application-like effective latency. The same defaults are inherited by standalone `-analyze-tlb` unless explicitly overridden.
  - **Auto TLB comparison now performs two explicit measurements (hit vs miss)**: When `-latency-tlb-locality-kb` is not user-specified, auto breakdown now executes a dedicated `16 KB` hit-biased pass and a dedicated `0` (global-random) miss-biased pass. Previously, the hit-biased value reused the headline pass. This keeps `TLB hit latency`, `TLB miss latency`, and `Estimated page-walk penalty` stable and unambiguous under the new default profile.
  - **Documentation synchronized with the new latency baseline**: Updated CLI help and docs to match default `-latency-stride-bytes 256` and default `-latency-tlb-locality-kb 1024` behavior (`README.md`, `MANUAL.md`, `LATENCY_WHITEPAPER.md`, `TECHNICAL_SPECIFICATION.md`, `TLB_ANALYSIS_WHITEPAPER.md`).
  - **`-analyze-tlb` startup output now prints active analysis settings**: Standalone TLB mode now prints a dedicated settings block before the sweep begins (CPU, page size, selected buffer, stride, requested/effective chain mode, loop count, sweep range, and page-walk comparison status). This makes it explicit which effective chain policy and measurement envelope are actually used in the run.
  - **Timer now enforces ASM kernel timing-contract fences (`dsb ish; isb`)**: `HighResTimer::start()`, `HighResTimer::stop()`, and `HighResTimer::stop_ns()` in `src/core/timing/timer.cpp` now emit `asm volatile("dsb ish\n\tisb" ::: "memory")` immediately before reading `mach_absolute_time()`. Every kernel under `src/asm/*.s` already documented this as a caller responsibility ("Caller must emit `dsb ish; isb` before reading the start/end timestamp"), but the timer was not honoring the contract, allowing Apple Silicon's out-of-order dispatch to straddle the timestamp read and the measured kernel body and introduce run-to-run jitter. Placing the fence inside the timer makes every current and future caller inherit the contract automatically. Headline means are not expected to shift; run-to-run variance should tighten. The `"memory"` clobber also forbids the C++ compiler from reordering memory accesses across the barrier.
  ...
  - **Harmonized 512B-loop tail paths across DRAM and cache ASM kernels**: DRAM read/write/copy tail handling converted to the `tbz`-bit-test tier shape already used by the cache kernels. Write and copy DRAM tails additionally gained scalar 16/8/4/2/1 fall-through tiers, removing their byte-by-byte fallback loops. Read DRAM kernel retains the byte-fold loop for correct XOR checksum granularity on the <32B residual. Bandwidth benchmark buffers are always 512B-aligned so these tail paths never run in practice; the change is maintenance-only to keep the two kernel families on a single tail idiom. Files: `src/asm/memory_read.s`, `src/asm/memory_write.s`, `src/asm/memory_copy.s`.
  - **Counted 512B hot loops in six main ASM kernels**: Replaced the `cmp + b.lo` top-of-loop guard plus `sub + b` unconditional back-edge with a precomputed iteration count (`lsr` + `and`) followed by `subs + b.ne` at the bottom. Removes 2 control instructions per 512B iteration from the hot path while preserving identical body and tail behavior. Block counter uses `x2` for the read/write kernels and `x3` for the copy kernels (where `x2` already carries the `byteCount` input); `x5` now holds tail bytes (remaining % 512) during the main loop. Clobber comments updated accordingly. Files: `src/asm/memory_read.s`, `src/asm/memory_write.s`, `src/asm/memory_copy.s`, `src/asm/memory_read_cache.s`, `src/asm/memory_write_cache.s`, `src/asm/memory_copy_cache.s`.
