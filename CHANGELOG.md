...
### Changed
  ...
  - **Machine consumers apply the complete result contract consistently**: Bundled schema-2 latency extractors require `status: "complete"`, `results_complete: true`, and `conclusions_valid: true`, while the manual's `jq -e` examples enforce every mode's schema and completeness predicate. The API and mode references also record observable stdout-transfer failures, zero-attempt sweep checkpoints, and the complete schema-1 sweep example.
