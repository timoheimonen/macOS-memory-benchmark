...
### Added
  - **Deterministic TLB planner and balanced round scheduler**: Standalone TLB analysis uses pure sweep-planning and measurement-scheduling modules. Each of 30 rounds measures every locality once in a seeded cyclic Latin order, rotating locality position across elapsed run time. `--seed <uint64>` reproduces planner order, derived task seeds, and standalone TLB pointer-chain permutations; one seed is generated per command when omitted.
  ...
  - **Validated standalone TLB JSON schema version 4**: TLB configuration metadata identifies `schema_version: 4`, `methodology_version: "page-native-paired-validated-v4"`, the translation-delta boundary signal, deterministic bootstrap method, minimum effect, persistence requirement, base seed/source, schedule policy, requested/effective chain modes, and macOS version. Execution-ordered records and point data separate discovery and validation measurements. Boundary objects contain the accepted bracket, discovery/validation evidence and 95% CIs, while accepted and rejected candidates remain available for diagnosis.
