...
### Changed
  ...
  - **Page-consistent standalone TLB locality grid**: `--analyze-tlb` requires a non-zero, pointer-aligned stride that does not exceed the system page size; exact page-size divisibility is unnecessary for the page-native chain. Fine-sweep refinement points are page-aligned and deduplicated, and planner pointer counts equal the requested active-page count.
  - **Neutral large-locality comparison terminology**: The standalone 512 MB comparison is reported as `large_locality_latency_delta` and explicitly described as a locality delta rather than an isolated page-table-walk cost. `page_walk_penalty` retains its documented legacy semantics as a deprecated alias through `0.57.x`, names `large_locality_latency_delta` as its replacement, and will not be removed before `0.58.0`. Legacy fixed `accesses_per_loop` is replaced by calibrated per-pair `access_count` values.
  - **TLB and sweep documentation synchronized with the shipped contract**: CLI help, README, manual, capabilities overview, parameter matrix, technical specification, TLB analysis whitepaper, JSON examples, and the analyzer plot script document page-native paired chains, stride constraints, seeded balanced rounds, independent validation, bootstrap confidence intervals, completion states, checkpoints, conclusion suppression, schema version 4, and compatibility fields.
