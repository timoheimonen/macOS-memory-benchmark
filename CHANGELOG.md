...
### Changed
  ...
  - **Flag-shaped output values remain opaque during primary-mode dispatch**: The primary-mode pre-scan consumes the `-o`/`--output` value before looking for mode selectors, so names such as `-G` remain files while genuine distinct mode options still conflict in either order. Dedicated core-to-core and GPU parser regressions also lock missing and duplicate output diagnostics to the centralized message contract.
