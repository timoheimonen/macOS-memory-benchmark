...
### Fixed
  - **Incremental builds track all project headers**: Makefile C++ object rules now depend on every `src/**/*.h` file, preventing stale objects from retaining an incompatible `BenchmarkConfig` or result-structure layout after header changes.
