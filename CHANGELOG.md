...
### Added
  ...
  - **Validated standalone TLB JSON schema version 4**: TLB configuration metadata identifies `schema_version: 4`, `methodology_version: "page-native-paired-adaptive-validated-v4"`, runtime profile and calibration policy, adaptive round bounds, work and memory estimates, best-effort buffer-lock status, the translation-delta boundary signal, deterministic bootstrap method, base seed/source, schedule policy, requested/effective chain modes, and macOS version. Execution-ordered records include per-chain pilot timing, calibrated accesses, physical diagnostics, and paired data. Boundary objects contain the accepted bracket, discovery/validation evidence and 95% CIs, while accepted and rejected candidates remain available for diagnosis.
