...
This document explains how `macOS-memory-benchmark` version 0.59.0 measures memory latency on Apple Silicon.
...
- The current standard schema 2 does not serialize the legacy `chain_diagnostics.unique_pages_touched` blocks. Do not
  use the old `main_memory.latency.chain_diagnostics` or `cache.*.latency.chain_diagnostics` paths for version 0.59.0
  output.
