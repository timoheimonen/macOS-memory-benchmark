...
int main(int argc, char *argv[]) {
  ...
  // --- Parse Command Line Args (early for cache size check) ---
  ...
  // Loop through arguments to get custom_cache_size_kb_ll early
  for (int i = 1; i < argc; ++i) {
    std::string arg = argv[i];
    try {
      // Handle -cache-size N (parse early for cache size detection)
      if (arg == "-cache-size") {
        if (++i < argc) {
          long long val_ll = std::stoll(argv[i]);  // Read value
          // Validate: must be positive and within reasonable bounds
          // Minimum is 16 KB (page size on macOS), maximum is 512 MB = 524288 KB
          ...
          custom_cache_size_kb_ll = val_ll;  // Store requested cache size in KB
        } else
  ...
  bool use_custom_cache_size = (custom_cache_size_kb_ll != -1);
  ...
  if (use_custom_cache_size) {
    // Convert custom cache size from KB to bytes
    custom_cache_size_bytes = static_cast<size_t>(custom_cache_size_kb_ll) * 1024;
  } else {
    // Get automatic cache sizes only if custom size is not set
    l1_cache_size = get_l1_cache_size();  // Get L1 cache size
    l2_cache_size = get_l2_cache_size();  // Get L2 cache size
  }
  ...
  size_t l2_buffer_size = 0;
  size_t custom_buffer_size = 0;
  size_t page_size_check = getpagesize();
  
  if (use_custom_cache_size) {
    // Use 100% of custom cache size
    custom_buffer_size = custom_cache_size_bytes;
    
    // Ensure buffer size is multiple of stride (128 bytes)
    custom_buffer_size = ((custom_buffer_size / lat_stride) * lat_stride);
    
    // Ensure minimum size (at least 2 pointers worth)
    if (custom_buffer_size < lat_stride * 2) custom_buffer_size = lat_stride * 2;
    
    // Ensure buffer size is at least page size aligned
    if (custom_buffer_size < page_size_check) {
      // Warn user if size was rounded up due to page size constraint
      size_t original_size_kb = custom_cache_size_bytes / 1024;
      ...
      custom_buffer_size = page_size_check;
    }
  } else {
    // Use 75% of cache size for L1 and 10% for L2 to ensure fits within target level
    l1_buffer_size = static_cast<size_t>(l1_cache_size * 0.75);
    l2_buffer_size = static_cast<size_t>(l2_cache_size * 0.10);
  ...
  // --- Parse Command Line Args (full parsing) ---
  // Loop through arguments again for all other options
  for (int i = 1; i < argc; ++i) {
    std::string arg = argv[i];
    try {
      // Handle -iterations N
      if (arg == "-iterations") {
        ...
        // Handle -cache-size N (already parsed, skip)
  ...
  size_t custom_num_accesses = 100 * 1000 * 1000;  // 100M accesses for custom cache
  ...
  print_cache_info(l1_cache_size, l2_cache_size, use_custom_cache_size, custom_cache_size_bytes);
  ...
  MmapPtr custom_buffer_ptr(nullptr, MmapDeleter{0});  // Custom cache latency test buffer
  MmapPtr custom_bw_src_ptr(nullptr, MmapDeleter{0});  // Custom cache bandwidth test buffer (source)
  MmapPtr custom_bw_dst_ptr(nullptr, MmapDeleter{0});  // Custom cache bandwidth test buffer (destination)
  ...
  if (use_custom_cache_size) {
    ...
    if (custom_buffer_size > 0) {
      void *temp_custom = mmap(nullptr, custom_buffer_size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
      if (temp_custom == MAP_FAILED) {
        perror("mmap failed for custom_buffer");
        return EXIT_FAILURE;
      }
      custom_buffer_ptr = MmapPtr(temp_custom, MmapDeleter{custom_buffer_size});
      if (madvise(temp_custom, custom_buffer_size, MADV_WILLNEED) == -1) {
        perror("madvise failed for custom_buffer");
      }
    }
  } else {
  ...
  if (use_custom_cache_size) {
    ...
    if (custom_buffer_size > 0) {
      ...
      void *temp_custom_bw_src = mmap(nullptr, custom_buffer_size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
      if (temp_custom_bw_src == MAP_FAILED) {
        perror("mmap failed for custom_bw_src_buffer");
        return EXIT_FAILURE;
      }
      custom_bw_src_ptr = MmapPtr(temp_custom_bw_src, MmapDeleter{custom_buffer_size});
      if (madvise(temp_custom_bw_src, custom_buffer_size, MADV_WILLNEED) == -1) {
        perror("madvise failed for custom_bw_src_buffer");
      }
      // Allocate destination buffer for custom cache bandwidth tests
      void *temp_custom_bw_dst = mmap(nullptr, custom_buffer_size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
      if (temp_custom_bw_dst == MAP_FAILED) {
        perror("mmap failed for custom_bw_dst_buffer");
        return EXIT_FAILURE;
      }
      custom_bw_dst_ptr = MmapPtr(temp_custom_bw_dst, MmapDeleter{custom_buffer_size});
      if (madvise(temp_custom_bw_dst, custom_buffer_size, MADV_WILLNEED) == -1) {
        perror("madvise failed for custom_bw_dst_buffer");
      }
    }
  } else {
  ...
  void *custom_buffer = custom_buffer_ptr.get();    // Get raw pointer for custom cache latency test buffer
  void *custom_bw_src = custom_bw_src_ptr.get();   // Get raw pointer for custom cache bandwidth test buffers
  void *custom_bw_dst = custom_bw_dst_ptr.get();
  ...
  if (use_custom_cache_size) {
    if (custom_buffer_size > 0 && custom_buffer != nullptr) {
      setup_latency_chain(custom_buffer, custom_buffer_size, lat_stride);
    }
  } else {
    if (l1_buffer_size > 0) {
  ...
  if (use_custom_cache_size) {
    if (custom_buffer_size > 0 && custom_bw_src != nullptr && custom_bw_dst != nullptr) {
      initialize_buffers(custom_bw_src, custom_bw_dst, custom_buffer_size);
    }
  } else {
  ...
  if (loop_count > 0) {
    ...
    if (use_custom_cache_size) {
      if (custom_buffer_size > 0) {
        all_custom_latency_ns.reserve(loop_count);
        all_custom_read_bw_gb_s.reserve(loop_count);
        all_custom_write_bw_gb_s.reserve(loop_count);
        all_custom_copy_bw_gb_s.reserve(loop_count);
      }
    } else {
  ...
  for (int loop = 0; loop < loop_count; ++loop) {
    ...
    double custom_lat_time_ns = 0.0, custom_latency_ns = 0.0;
    ...
    double custom_read_time = 0.0, custom_read_bw_gb_s = 0.0;
    double custom_write_time = 0.0, custom_write_bw_gb_s = 0.0;
    double custom_copy_time = 0.0, custom_copy_bw_gb_s = 0.0;
    ...
    try {
      ...
      if (use_custom_cache_size) {
        ...
        if (custom_buffer_size > 0 && custom_bw_src != nullptr && custom_bw_dst != nullptr) {
          ...
          warmup_cache_read(custom_bw_src, custom_buffer_size, single_thread, custom_warmup_read_checksum);
          custom_read_time = run_read_test(custom_bw_src, custom_buffer_size, cache_iterations, single_thread, custom_read_checksum, test_timer);
          warmup_cache_write(custom_bw_dst, custom_buffer_size, single_thread);
          custom_write_time = run_write_test(custom_bw_dst, custom_buffer_size, cache_iterations, single_thread, test_timer);
          warmup_cache_copy(custom_bw_dst, custom_bw_src, custom_buffer_size, single_thread);
          custom_copy_time = run_copy_test(custom_bw_dst, custom_bw_src, custom_buffer_size, cache_iterations, single_thread, test_timer);
        }
      } else {
    ...
    if (use_custom_cache_size) {
      if (custom_buffer_size > 0) {
        size_t custom_total_bytes_read = static_cast<size_t>(cache_iterations) * custom_buffer_size;
        size_t custom_total_bytes_written = static_cast<size_t>(cache_iterations) * custom_buffer_size;
        size_t custom_total_bytes_copied_op = static_cast<size_t>(cache_iterations) * custom_buffer_size;
        if (custom_read_time > 0) custom_read_bw_gb_s = static_cast<double>(custom_total_bytes_read) / custom_read_time / 1e9;
        if (custom_write_time > 0) custom_write_bw_gb_s = static_cast<double>(custom_total_bytes_written) / custom_write_time / 1e9;
        if (custom_copy_time > 0) custom_copy_bw_gb_s = static_cast<double>(custom_total_bytes_copied_op * 2) / custom_copy_time / 1e9;
      }
    } else {
      if (l1_buffer_size > 0) {
    ...
    if (use_custom_cache_size) {
      if (custom_buffer_size > 0) {
        all_custom_latency_ns.push_back(custom_latency_ns);
        all_custom_read_bw_gb_s.push_back(custom_read_bw_gb_s);
        all_custom_write_bw_gb_s.push_back(custom_write_bw_gb_s);
        all_custom_copy_bw_gb_s.push_back(custom_copy_bw_gb_s);
      }
    } else {
      if (l1_buffer_size > 0) {
    ...
    print_results(loop, buffer_size, buffer_size_mb, iterations, num_threads, read_bw_gb_s, total_read_time,
                  ...
                  use_custom_cache_size, custom_latency_ns, custom_buffer_size,
                  custom_read_bw_gb_s, custom_write_bw_gb_s, custom_copy_bw_gb_s);
  ...
  print_statistics(loop_count, all_read_bw_gb_s, all_write_bw_gb_s, all_copy_bw_gb_s,
                   ...
                   use_custom_cache_size,
                   all_custom_latency_ns, all_custom_read_bw_gb_s,
                   all_custom_write_bw_gb_s, all_custom_copy_bw_gb_s);
