...
| JSON output (`--output`) and sweeps (`--sweep`) | Auditable measurement evidence through recoverable files or, for direct standard/pattern commands, one final stdout document | Comparability when commands, software, hardware, or run conditions differ |
...
Direct standard and pattern commands reserve the exact target `--output -` for machine-readable output: stdout contains
one final JSON document and the post-parse human transcript is routed to stderr. `--output ./-` remains an ordinary file
target. Real files preserve their documented atomic persistence behavior. TLB, core-to-core, GPU, and all sweep commands
remain file-only in this revision; the current support matrix and process acceptance procedure are in the
[Machine-Readable CLI API](API.md).
