...
- `-sweep <key=a,b>`: Sweep supported parameters. General benchmark keys: `buffersize`, `cache-size`, `threads`, `latency-tlb-locality-kb`, `latency-stride-bytes`, `latency-chain-mode`; TLB analysis key: `tlb-density`; core-to-core keys: `count`, `latency-samples`.
