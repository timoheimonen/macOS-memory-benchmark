...
- **`--analyze-tlb`**: Runs standalone TLB analysis with page-native spread/packed pairs, balanced seeded rounds, automatic refinement, paired bootstrap confidence intervals, and an independent boundary-validation pass; only optional `--output <file>`, `--latency-stride-bytes <bytes>`, `--latency-chain-mode <mode>`, `--tlb-density <low|medium|high>`, `--seed <uint64>`, `--sweep <key=...>`, and `--sweep-max-runs <count>` may be combined with it.
...
- `--analyze-tlb`: Standalone TLB-boundary benchmark (`1024/512/256 MB` fallback buffer selection), sweeping page-aligned locality windows from `max(16 KB, 2*stride)` to `256 MB` with fine refinement (plus an optional `512 MB` comparison). Every scheduled point is a same-round spread/packed pair, and 30 seeded cyclic-Latin rounds balance elapsed-time drift. Boundary inference operates on round-matched `spread - packed` deltas. A candidate is accepted only when its paired effect is at least `0.5ns`, its deterministic bootstrap 95% CI clears the measured noise floor, both following points confirm persistence, and an independent validation pass repeats the result. The entry range from the final measured bracket is primary; the midpoint is only an estimate. Stride must be pointer-aligned and no larger than the system page size; it need not divide the page size.
...
Standalone TLB JSON uses `schema_version: 4` and `methodology_version: "page-native-paired-validated-v4"`. It includes the base seed, schedule and chain metadata, execution-ordered discovery/validation records, raw pair latencies, verified physical diagnostics, and same-round `translation_delta_ns = spread - packed`. `configuration.boundary_signal` is `translation_delta_ns`. Boundary objects retain accepted and rejected candidates with discovery/validation evidence, deterministic bootstrap 95% CIs, noise floor, persistence counts, rejection reasons, and the final bracket. Completion metadata separately reports validation progress. The preferred large-locality block remains `tlb_analysis.large_locality_latency_delta`; `tlb_analysis.page_walk_penalty` is a deprecated compatibility alias.
