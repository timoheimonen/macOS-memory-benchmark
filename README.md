...
- **`--analyze-tlb`**: Runs standalone TLB analysis mode with page-native spread/packed control pairs, automatic two-stage sweep refinement, balanced seeded measurement rounds, private-cache-knee detection, and L1/L2 TLB boundary inference; only optional `--output <file>`, `--latency-stride-bytes <bytes>`, `--latency-chain-mode <mode>`, `--tlb-density <low|medium|high>`, `--seed <uint64>`, `--sweep <key=...>`, and `--sweep-max-runs <count>` may be combined with it.
...
- `--analyze-tlb`: Standalone TLB-boundary detection benchmark (`1024/512/256 MB` fallback buffer selection), sweeping page-aligned locality windows from `max(16 KB, 2*stride)` to `256 MB` with fine-sweep insertion near detected knees/boundaries (plus an optional `512 MB` large-locality comparison when buffer is at least `512 MB`). Each scheduled point is a same-round pair: spread uses exactly one pointer node per requested page, while packed uses the same node and unique-cache-line counts in a cache-line-dense footprint. Each of 30 rounds measures every locality once using a seeded cyclic Latin order, reducing correlation between locality and run-time drift. Reports private-cache-knee risk, private-cache/L1 overlap when ambiguous, and inferred entry estimates plus ranges. Stride must be pointer-aligned and no larger than the system page size; it need not divide the page size. Supports optional `--latency-stride-bytes <bytes>`, `--latency-chain-mode <mode>`, `--tlb-density <low|medium|high>`, `--seed <uint64>`, and sweep mode over those same three TLB-analysis parameters.
...
Standalone TLB JSON uses `schema_version: 3` and `methodology_version: "page-native-paired-v3"`. It includes the base seed, seed source, schedule policy, pass/round/order records, both raw pair latencies, layout-specific seeds, alternating pair order, verified physical-chain diagnostics, and same-round `translation_delta_ns = spread - packed`. Per-point records include raw and P50 spread/packed/delta values plus actual page/node/cache-line counts. `latency_ns` and boundary inference remain spread-based until the phase-4 changepoint model is implemented; `configuration.boundary_signal` makes this explicit. Completion metadata distinguishes scheduled pairs from raw measurements. The preferred large-locality comparison block is `tlb_analysis.large_locality_latency_delta`; the former `tlb_analysis.page_walk_penalty` block remains for one compatibility window with `deprecated: true` and a `replacement` field.
